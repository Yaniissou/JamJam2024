...
from objects.ChargeState import ChargeState
from objects.Col_active import Col
import random
...
player2 = Player(145, 680,competences,60,None,images_sprite_france,0)
...
def checkItemCollisions(player, items):
    for item in items:
        if player.rect.colliderect(item.rect):
            ...
            items.remove(item)
        elif player2.rect.colliderect(item.rect):
            player2.etoile += 40
            print("Collision detected!")
            items.remove(item)
...
def inGame():
    ...
    player2.country = selected_country
    player2.competences = player2.country.competences

    if player.country == allemagne:
        player.images = images_sprite_allemagne
    elif player2.country == allemagne:
        player2.images = images_sprite_allemagne

    titletext = titlefont.render(f"nb etoile :  {player.etoile}", False, (0, 0, 0))
    titletext_rect = titletext.get_rect()
    titletext_rect.center = (window_width / 2, window_height / 8)

    titletext2 = titlefont.render(f"nb etoile2 :  {player2.etoile}", False, (0, 0, 0))
    titletext_rect2 = titletext2.get_rect()
    titletext_rect2.center = (window_width / 2, window_height / 6)
    ...
    previous_position_2 = player2.rect.copy()
    player.deplacer()
    player2.deplacer()
    for tile in collision_tiles:
        if player.rect.colliderect(tile):
            player.rect = previous_position
        elif player2.rect.colliderect(tile):
            player2.rect = previous_position_2
    window.blit(player.image, player.rect)
    window.blit(player2.image,player2.rect)
    window.blit(titletext,titletext_rect)
    window.blit(titletext2,titletext_rect2)
